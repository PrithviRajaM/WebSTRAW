...
import * as nativeBridge from "./native-bridge.js";
...
browser.runtime.onMessage.addListener((message, sender) => {
	...
	if (message.method.startsWith("nativeBridge.")) {
		if (message.method == "nativeBridge.enable") {
			nativeBridge.enable();
		} else if (message.method == "nativeBridge.disable") {
			nativeBridge.disable();
		}
		return Promise.resolve({ enabled: nativeBridge.isEnabled() });
	}
